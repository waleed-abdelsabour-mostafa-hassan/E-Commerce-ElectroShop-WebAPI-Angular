...
export class ProdectDetailsComponent implements OnInit {
  ...
  addNewComment(
    ...
    this.Comments.AddNewComment(this.userID, ProdID, Comment).subscribe(e=>{
  ...
  addNewFavoritProduct(favProdiID: number) {
    this.favoProd.addnew(favProdiID,this.userID).subscribe();
    location.reload();
  }
  userID="";
  imgg="";
  ...
  AddCard(idproduct :number,inpuvalue:string){
    ...
    this.cardService.AddCartByDetails(idproduct,valuecount ,this.userID).subscribe(
      (e) => {
        this.countitemcard();
        this.tost.success({
